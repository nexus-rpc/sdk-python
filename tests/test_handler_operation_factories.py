...
from nexusrpc.handler._core import collect_operation_handler_methods
...
async def test_collected_operation_factories_match_service_definition(
    ...
    operation_factories = collect_operation_handler_methods(test_case.Service)
    ...
    async def execute(
        op: nexusrpc.handler.OperationHandler[I, O],
        ctx: nexusrpc.handler.StartOperationContext,
        input: I,
    ) -> Union[
        nexusrpc.handler.StartOperationResultSync[O],
        nexusrpc.handler.StartOperationResultAsync,
    ]:
        if inspect.iscoroutinefunction(op.start):
            return await op.start(ctx, input)
        else:
            return cast(
                nexusrpc.handler.StartOperationResultSync[O], op.start(ctx, input)
            )
